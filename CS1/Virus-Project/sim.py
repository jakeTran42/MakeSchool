...
class Simulation(object):
    ...
    def _create_population(self, initial_infected):
        ...
        while len(population) != self.population_size:
            if infected_count !=  initial_infected:
                _id = len(population)
                infectedPerson = Person(_id, False, True)
                self.newly_infected.append(_id)
                population.append(infectedPerson)
                infected_count += 1
                ...
                if(random.random() < vacc_percentage):
                    ...
                    new_vaccinated = False
                vacPerson = Person(_id, new_vaccinated, False)

                population.append(vacPerson)
    ...
    def run(self):
        ...
        while should_continue:
            ...
            self.logger.log_time_step(time_step_counter)
            should_continue = self._simulation_should_continue()
        print("Simulation ended at %s" %(time_step_counter))
    ...
    def interaction(self, person, random_person):
        ...
        self.logger.log_interaction(person, random_person, already_infect)
    ...
    def _infect_newly_infected(self):
        for sick_id in self.newly_infected:
            self.population[sick_id].infected = True
        self.newly_infected = []
